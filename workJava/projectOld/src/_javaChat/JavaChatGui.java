...
implements Runnable {
	...
	private int foreColor = Color.BLACK.getRGB();//private float foreColor = Color.BLACK.getRGB();// 

	boolean isStart;// 쓰레드 처리
	private StyledDocument doc;// 채팅창 글짜 색깔입히기 위해서 
	
    public JavaChatGui() {
        initComponents();
//----------------------------------------------------------------
        doc = taMsg.getStyledDocument();
...
//    }//
    ...
    private void parsing(String serverMsg) {
    	...
    	switch(protocol){
	    	...
	    	case 400 : {
	    		...
	    		//showTaMsg( fromChat + ">>" + message);// textArea 사용시
	    		showTextPane(fromChat, fontColor, message);
	    	}
	    	break;
	    	case 800 : {
	    		showMsg("대화명이 중복됩니다.\n다시 로그인하세요.");
    ...
    // 채팅상자 - 글자색 변경하기 위해서 처리함
    private void showTextPane(String fromChat, String fontColor, String message) {

    	int foRGB = Integer.parseInt(fontColor);
    	SimpleAttributeSet attr = new SimpleAttributeSet();
    	StyleConstants.setForeground(attr, new Color(foRGB));
    	setTitle(fontColor);
    	
    	String msg = "[" + fromChat + "]님 >> " + message + "\n";
    	
    	taMsg.setCaretPosition(
    			doc.getEndPosition().getOffset() -1
    	);// 스크롤바 캐랫포지션 잡아준다.
    	
    	int end = taMsg.getCaretPosition();// 위치를 얻어와서 넣어준다.
    	try {
...
//		tabP.setEnabledAt(1, false);
	...
	private void showTaMsg(String msg) {
		//tfChat.setText(msg);
    	//msg = tfChat.getText();
		taMsg.setText(taMsg.getText() + msg+"\n");
		//taMsg.append(msg+"\n");
...
//		///JOptionPane.showMessageDialog(this, msg);
    ...
    private void comboColorItemStateChanged(ItemEvent evt) {
    	...
    	switch(index){
    	case 0: 
    		foreColor = Color.black.getRGB();
    		break;// 검정
    	case 1: 
    		foreColor = Color.blue.getRGB();
    		break;// 파랑
    	case 2: 
    		foreColor = Color.red.getRGB();
    		break;// 빨강
    	case 3: 
    		foreColor = Color.green.getRGB();
    		break;// 초록
    	case 4: 
    		foreColor = Color.pink.getRGB();
    		break;// 핑크
    	case 5: 
    		foreColor = Color.orange.getRGB();
    		break;// 오렌지색
    	}
    	
    	lbView.setForeground( new Color(foreColor) );
    }// private void comboColorItemStateChanged(ItemEvent evt)
