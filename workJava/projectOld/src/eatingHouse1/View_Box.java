 ...
 *  채팅기능 - thread 동기화 기능  
 *  이미지 에디터 라이브러리 구현
 *  <차후 UX : 스마트폰(android,ios) - PC스윙 - 웹싸이트HTML > 유저인터페이스
 *  
 *  게시판 기능---------------
 *	회원그룹관리연동인증시스템
 *	포럼형식의 기반게시판추구(그룹및개인자료실제공가능성)
 * 
 *  dataGetLoader - jdbc 기능
 *  기타 (RDBMs, Nosql, Sqlite, user.rdat)
 *  
 *  리스트 업데이트 기능  *    
 *  UCan : 현재 텍스트 파일을 임시 디비로 만들계획
 *  UCan : 게시판 하단 번호페이지 카운터(1,2,3,..8,9,10)까지 구현 - 변수줘서 조절가능
 *  UCan : 게시판 하단 번호페이지 좌우 10개 30개 건너뛰기 네이비게이션 구현 - 변수줘서 조절가능
 *  UCan : 향후 RDBMs(오라클,Mysql,Mssql등),NoSql,Mongo,In-memory,Sqlite 연결클래스에 Db데이타 로드할 목적으로 만든 함수
 *  UCan : 참조나 데이타를 끌어오기 위한 참조구현
 *  ------------------------------------------------------------
 *  WeCanDoIt.
 *  ------------------------------------------------------------
 *  
 ...
 *	관리자 프로그램
 *	실증인증서비스(월기본료의이용건수당)
 *	각종API 정보 이용서비스료(OneDomain & OneServer OR 기본정액제 AND OR 몇쿼리당 OR TimeSection)
 *	SMS문자서비스(서비스관련회사계약) - 아마존 SMS 서비스 api - 가격이 싸다.
 *	전자결제(신용카드,온라인,핸드폰,전자머니형태,기타) - 회원연동지불연동시스템
 *	결제및 기타 타URL연동된 데이타베이스관리 프로그램
 *	홈페이지내 DB시스템 관리서비스
 *	
 *
 *  기타:
 ...
 *  주문,오더견적폼
 *  스케쥴관련프로그램
 *  카운터지원(기능성카운터,월별,일별,시간대별접속통계및 기타로그지원)
 *  기능성 공개형 게시판지원(스펨게시물차단어기능,권한부여기능기타등등)
 *  전자우편(웹메일필요가능성)지원
 *  홈페이지(일반독립, SNS, BLOG, 대한민국전자정부) 연동 - 유지보수및 관리지원필요
 *  독립서버내지 호스팅에 관련되 보안그룹
 *  각종 API 써비스 (한국정부API단체, 구글, 트윗, 페팔, 페이스북, 네이버, 다음) 연동 지원
 *  
 */
...
public class View_Box extends JFrame {
	private static View_Box win;						// 레이아웃 틀 View_Box
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					win = new View_Box();						// View_Box.frame 객체
					win.shopList =new ShopMemberList();		// 상점리스트 자동초기화
					...
					// View_Box.frame에 손님메인화면 교체 초기화			// win.view_1 = new View_1(win);					//win.view_2 = new View_2(win);					//win.add(win.view_1);					// View_Box에 손님메인화면 교체
				} catch (Exception e) {
					e.printStackTrace();
				}
			} // public void run()
	...
	public View_Box() {
		...
		this.setIconImage(Toolkit.getDefaultToolkit().getImage("images/logo.png"));		// View_Box.frame 상단아이콘		
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);								// View_Box.frame 닫기시 종료
		this.setBounds(100, 100, 800, 600);														// View_Box.frame window내 위치
		...
		this.contentPane = new JPanel();															// View_Box.frame 레이아웃패널관리매니저 초기화
		this.contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));								// View_Box.frame 레이아웃패널관리매니저 공백보더지정
		this.contentPane.setLayout(new BorderLayout(0, 0));									// View_Box.frame 레이아웃패널관리매니저 레이아웃매니저 지정
		this.setContentPane(this.contentPane);														// View_Box.frame의 레이아웃패널관리매니저 지정
...
class ShopMemberList{
	...
	public void dataGetLoader(){       
        ...
        shopmem.addElement(new Member(11, "s11", "1234", "s11.PNG", "시로우마", "일식", "02-0268-8652", "서울 영등포구 양평동4가 278-3 1층", "대중교통 이용시\n당산역 3번출구로 오셔서 앞으로 쭉 도보 100m 걸어 오시면 됩니다.\n\n자가용 이용시\n선유도역 방면으로 양평동 사거리에서 좌회전해서 골목으로 들어오시면 됩니다.", 5, "./m11.PNG","오늘의 초밥(12pcs),16000|모듬초밥(14pcs),21000|연어초밥(14pcs),51000", "f11_1.PNG|f11_2.PNG|f11_3.PNG"));
	...
	public void setInsertMember(Member insertMember){	// 회원 추가 정보
		this.insertmember = insertMember;
	}
	public Member getInsertMember(){	// 회원 추가된 정보
