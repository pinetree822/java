...
class View_2 extends JPanel {
	...
	public View_2(View_Box win) {							// View_2 생성자의 인자로 온 Win_Box win
		...
		/// 상점자동리스트
		///////////////////////////////////////////////////////////////////////////
		shopList = new ShopMemberList();	// 상점자동리스트
		shopList.dataGetLoader();				// 상점데이타 리스트 로딩
		if( shopList.getNowpage() <= 1 ) {
			shopList.setNowpage(1);				// 첫페이지 로딩
		}
		shopList.previewList(win,sPanel_2_1);	// 상점데이타로 리스트를 화면 프린트
		...
		btn3.addActionListener( new MyActionListener() );		// 상점등록 button
	...
	/// 화면구상도의 패널구성 목록에서 "view_2" 형식으로 전달한다.
	...
	class MyActionListener implements ActionListener, KeyListener {
		...
		public void actionPerformed(ActionEvent e) { 			// ActionListener에서 이벤트 발생하면 자동호출되는 ActionEvent 인자값을 갖는 actionPerformed함수
			...
			if(o==btn2 ){				// 검색 button
				...
				win.change("view_1_1");
			} else if(o==btn3 ){		// 상점등록 button
				JOptionPane.showMessageDialog(null, "상점등록으로 이동합니다.");	// view_5
				...
				shopList = new ShopMemberList();				//상점자동리스트 - 실시간 관리가 필요할때 변화는 자료 계속 읽음
				shopList.dataGetLoader();							// 초기화 리스트 데이타 리스트 재로딩
				shopList.setNowpage(shopList.getFirstpage());	// 처음페이지 얻어서 현재 페이지 설정
				shopList.previewList(win,sPanel_2_1);				// 현재페이지 기준으로 페이지리스트 프린트
	
			} else if(o==lbtn2) {	//이전버튼
				JOptionPane.showMessageDialog(null, "이전 클릭.");
				int tmpPrevBtn = shopList.getPrevpage();		// ShopMemberList를 초기화 시켜서 이전에 값을 받아서 저장해서 Prev페이지 네이비게이션 값으로 쓴다.
				shopList = new ShopMemberList();				// 상점자동리스트
				shopList.dataGetLoader();							// 초기화 리스트 데이타 리스트 재로딩
				shopList.setNowpage(tmpPrevBtn);				// 이전페이지 얻어서 현재 페이지 설정
				shopList.previewList(win,sPanel_2_1);				// 현재페이지 기준으로 페이지리스트 프린트
				
			} else if(o==lbtn3) {	//다음버튼
				JOptionPane.showMessageDialog(null, "다음 클릭.");
				int tmpNextBtn = shopList.getNextpage();		// ShopMemberList를 초기화 시켜서 이전에 값을 받아서 저장해서 Next페이지 네이비게이션 값으로 쓴다.
				shopList = new ShopMemberList();				// 상점자동리스트
				shopList.dataGetLoader();							// 초기화 리스트 데이타 리스트 재로딩
				shopList.setNowpage(tmpNextBtn);				// 처음페이지 얻어서 현재 페이지 설정
				shopList.previewList(win,sPanel_2_1);				// 현재페이지 기준으로 페이지리스트 프린트
				
			} else if(o==lbtn4) {	//끝버튼
				/* 변수확인 경고창
						JOptionPane.showMessageDialog(null, "검증 = shopList.getAllpage = " + (shopList.getAllpage()*1) + ", 검증 = shopList.getLastpage = " + shopList.getLastpage()*1 + ", 검증 = shopList.getNowpage = " + shopList.getNowpage()*1);			// view_1_1_1
				 */
				JOptionPane.showMessageDialog(null, "끝페이지 클릭.");
				shopList = new ShopMemberList();				// 상점자동리스트
				shopList.dataGetLoader();							// 초기화 리스트 데이타 리스트 재로딩
				shopList.setNowpage(shopList.getLastpage());	// 처음페이지 얻어서 현재 페이지 설정
				shopList.previewList(win,sPanel_2_1);				// 현재페이지 기준으로 페이지리스트 프린트
			} else {
			}
