	...
	public void run() {
		while( running ) {
			try {
				...
				System.out.println("SocketTimeout SocketTimeoutException-----------index = " + index--);
				e.printStackTrace();
				
			} catch (InterruptedException e) {
				// http://nearhomedeveloper.tistory.com/entry/JavaThread-Java-Thread-%EB%B0%94%EB%A1%9C-%EC%A2%85%EB%A3%8C-%EC%8B%9C%ED%82%A4%EA%B8%B0-%E2%80%93-interrupt
				// 참고
				index--;// 동기화******
				e.printStackTrace();
				
			} catch (IOException | ClassNotFoundException e) {
				System.out.println("SocketTimeout IOException | ClassNotFoundException-----------index = " + index--);
				e.printStackTrace();
				
			} catch ( Exception e) {
				System.out.println("SocketTimeout Exception-----------index = " + index--);
				e.printStackTrace();
