...
public class join extends JFrame  {
	...
	public join() {
		super("::회원가입::");
		GridLayout g1=new GridLayout(8,0);	// 레이아웃매니저객체 g1 초기화
		setLayout(g1);						// join 레이아웃 설정

		JPanel p1=new JPanel();								// p1 패널 초기화
		p1.setLayout(new FlowLayout(FlowLayout.LEFT));	// p1 레이아웃 설정 - FlowLayout() 에 상수인 LEFT로 정렬 설정
		add(p1);													// join 프레임에 p1 패널 추가
		lb1=new Label("아     이     디"); this.add(lb1);	// 레이블 아이디 설정 하고 join 프레임에 추가
		tf1=new TextField(10); this.add(tf1);					// tf1 텍스트필드이고 칸수 10개인 만들고 join 프레임에 추가
		bt3=new Button("중복확인");							// bt3 버튼 초기화(중복확인)
		p1.add(lb1);												// p1 패널에 lb1(아이디) 추가
		p1.add(tf1);												// p1 패널에 tf1(아이디텍스트필드) 추가
		p1.add(bt3);												// p1 패널에 bt3(중복확인버튼) 추가
	...
	/*
		class MyHandler1 implements ActionListener{
			...
			public void actionPerformed(ActionEvent r){
				String cmd=r.getActionCommand();		
				if(cmd.equals(btnDispose));				//	String btnDispose =;	
				dispose();		
			}
		}
	*/
