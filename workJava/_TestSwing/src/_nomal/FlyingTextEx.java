...
public class FlyingTextEx extends JFrame{
	...
	public FlyingTextEx() {
		...
		c.addKeyListener(new MyKeyListener());// 컨텐트팬에 Key리스너 달기
		...
		c.setFocusable(true);	// 컴포넌트에게 포커스 설정
		c.requestFocus();// 컨텐트팬이 키 입력을 받을 수 있도록 포커스 강제 지정
		
		// 다음 코드는 컨텐트팬에 포커스를 잃은 경우 
		// 마우스 클릭하면 다시 포커스를 얻게 함
		c.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				c.setFocusable(true);	// 컴포넌트에게 포커스 설정
				c.requestFocus();		// 컴포넌트에게 키 입력을 받을 수 있도록 포커스 강제 지정
			}
		});
	}// FlyingTextEx()
